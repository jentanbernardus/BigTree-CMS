		...
		$(".pop-dependant").each(function(el) {
			if ($(this).hasClass(name)) {
				if ($(this).hasClass("sort_by")) {
					$(this).load("<?=$aroot?>ajax/developer/load-table-columns/?sort=true&table=" + table + "&field=" + $(this).attr("name"));
				} else {
					$(this).load("<?=$aroot?>ajax/developer/load-table-columns/?table=" + table + "&field=" + $(this).attr("name"));
				}
			}
		});
	});
