<?
	...
	$types = $admin->getFoundryFieldTypes();
...
<dl class="table" id="sort_table">
	<dt>
		<span class="foundry_name"><a href="<? if ($sort == "name" && $sort_direction == SORT_ASC) { ?>SORT_DESC<? } else { ?>SORT_ASC<? } ?>" class="sort_column" name="name">Field Type <? if ($sort == "name" && $sort_direction == SORT_ASC) { ?>&#9650;<? } elseif ($sort == "name") { ?>&#9660;<? } ?></a></span>
	...
	<dd>
		<ul>
			...
			<li class="action">
				<? if ($f = sqlfetch(sqlquery("SELECT * FROM bigtree_field_types WHERE foundry_id = '".mysql_real_escape_string($item["id"])."'"))) { ?>
