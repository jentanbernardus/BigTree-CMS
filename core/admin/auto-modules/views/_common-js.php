<script type="text/javascript">
	...
	$(".icon_delete").live("click",function() {
		new BigTreeDialog("Delete Item",'<p class="confirm">Are you sure you want to delete this item?</p>',$.proxy(function() {
			$.ajax("<?=$aroot?>ajax/auto-modules/views/delete/?view=<?=$view["id"]?>&id=" + BigTree.CleanHref($(this).attr("href")));
	...
	$(".icon_approve").live("click",function() {
		$.ajax("<?=$aroot?>ajax/auto-modules/views/approve/?view=<?=$view["id"]?>&id=" + BigTree.CleanHref($(this).attr("href")));
		$(this).toggleClass("icon_approve_on");
		return false;
	});

	$(".icon_feature").live("click",function() {
		$.ajax("<?=$aroot?>ajax/auto-modules/views/feature/?view=<?=$view["id"]?>&id=" + BigTree.CleanHref($(this).attr("href")));
		$(this).toggleClass("icon_feature_on");
		return false;
	});

	$(".icon_archive").live("click",function() {
		$.ajax("<?=$aroot?>ajax/auto-modules/views/archive/?view=<?=$view["id"]?>&id=" + BigTree.CleanHref($(this).attr("href")));
