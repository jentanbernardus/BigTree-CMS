...
<script type="text/javascript">
	$("#graph_data").load("<?=$admin_root?>ajax/analytics/get-page-graph/", { page: <?=end($path)?>, start_date: "<?=$tw_start?>", end_date: "<?=$tw_end?>" });
	$("#ga_current_month").load("<?=$admin_root?>ajax/analytics/get-page-date-range/", { page: <?=end($path)?>, title: "Current Month", start_date: "<?=date("Y-m-01")?>", end_date: "<?=date("Y-m-d")?>" });
	$("#ga_current_quarter").load("<?=$admin_root?>ajax/analytics/get-page-date-range/", { page: <?=end($path)?>, title: "Current Quarter", start_date: "<?=$quarter_start?>", end_date: "<?=$quarter_end?>" });
	$("#ga_current_year").load("<?=$admin_root?>ajax/analytics/get-page-date-range/", { page: <?=end($path)?>, title: "Current Year", start_date: "<?=$year_start?>", end_date: "<?=$year_end?>" });
</script>