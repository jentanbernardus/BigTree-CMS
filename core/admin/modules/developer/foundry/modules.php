...
<script type="text/javascript">
	...
	function swapSearch() {
		...
		$("#results").load("<?=$admin_root?>ajax/developer/foundry/module-page/?sort=" + escape(sort) + "&sort_direction=" + escape(sortdir) + "&page=0&search=" + search);
	}
	
	function swapPage(page) {
		mpage = page;
		$("#results").load("<?=$admin_root?>ajax/developer/foundry/module-page/?sort=" + escape(sort) + "&sort_direction=" + escape(sortdir) + "&search=" + escape(search) + "&page=" + mpage);
	}
	...
	$(".button_view").click(function() {
		$.ajax("<?=$admin_root?>ajax/developer/foundry/get-module-details/?id=" + BigTree.CleanHref($(this).attr("href")), { complete: function(r) {
	...
	$(".button_update").click(function() {
		document.location.href = "<?=$admin_root?>developer/foundry/update/module/" + BigTree.CleanHref($(this).attr("href")) + "/";
		
		return false;
	});
	
	$(".button_download").click(function() {
		document.location.href = "<?=$admin_root?>developer/foundry/install/module/" + BigTree.CleanHref($(this).attr("href")) + "/";
