<?	
	...
	$existing_package = sqlfetch(sqlquery("SELECT * FROM bigtree_module_packages WHERE foundry_id = '".mysql_real_escape_string(end($path))."'"));
	...
	// Clear out all pre-existing templates, sidelets, feeds, settings
	sqlquery("DELETE FROM bigtree_templates WHERE package = '$existing_id'");
	sqlquery("DELETE FROM bigtree_sidelets WHERE package = '$existing_id'");
	...
	$existing_details = unserialize($existing_package["details"]);
	...
	sqlquery("UPDATE bigtree_module_packages set author = '".mysql_real_escape_string($author["name"])."', name = '$name', primary_version = '$primary_version', secondary_version = '$secondary_version', tertiary_version = '$tertiary_version', description = '$description', release_notes = '$release_notes', details = '".mysql_real_escape_string(serialize($savedData))."', `tables` = '$package_tables', `files` = '$package_files', downloaded = 'on', last_updated = NOW() WHERE foundry_id = '$id'");
