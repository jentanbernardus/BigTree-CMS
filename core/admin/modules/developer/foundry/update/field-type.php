...
<?
	$existing = sqlfetch(sqlquery("SELECT * FROM bigtree_field_types WHERE id = '".mysql_real_escape_string($details["field_type_id"])."'"));
	$existing_files = unserialize($existing["files"]);
	...
	foreach ($lines as $line) {
		...
		if (file_exists($server_root.$file) && !in_array($file,$existing_files))
