...
<?
	if (!is_writable($server_root."cache/")) {
		...
		$existing_tables = unserialize($existing_package["tables"]);
		$existing_files = unserialize($existing_package["files"]);
		...
		foreach ($lines as $line) {
			...
			if ($type == "Sidelet") {
				$r = sqlrows(sqlquery("SELECT * FROM bigtree_sidelets WHERE package != '$existing_id' AND id = '".mysql_real_escape_string($data["id"])."'"));
				if ($r)
